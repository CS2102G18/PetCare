...
$query = "UPDATE request SET status='failed' WHERE status='pending' AND request_id NOT IN(
         SELECT r.request_id FROM request r,availability a
         WHERE a.is_deleted=false AND r.taker_id=" . $user_id . " AND a.taker_id=" . $user_id . " AND a.start_time<=r.care_begin AND r.care_end<=a.end_time);";
$result = pg_query($query) or die('Query failed: ' . pg_last_error());
pg_free_result($result);
