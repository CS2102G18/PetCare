    ...
    <div class="container-fluid">
        <div class="panel new-task-panel">
            ...
            $query = "SELECT u.name, COUNT(*), (SUM(r.bids)/SUM(r.totaltime))*60 AS avg FROM request r, pet_user u
                  WHERE r.owner_id = $user_id AND r.status = 'successful' AND u.user_id = r.taker_id
                  GROUP BY r.taker_id, u.name
                  HAVING COUNT(*) >= ALL(SELECT COUNT(*) FROM
                                         request r1
                                         WHERE r1.owner_id = $user_id AND r1.status = 'successful'
                                         GROUP BY r1.taker_id)
            ...
            <div class="box">
                <div class="container">
                    <h4><strong>Favorite Taker:</strong> <?php echo"$favoritename"; ?></h4>
                    <h4><strong>Number of successful Requests:</strong> <?php echo"$favoritetime"; ?></h4>
                    <h4><strong>Average Bids/Hour You provided:</strong> <?php echo"$favoriteavg"; ?></h4>
                </div>
            </div>
